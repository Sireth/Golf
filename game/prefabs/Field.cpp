...
enum UserData { BALL = 5, WALL = 6 };

Field::Field(bool isLeft, uint level, bool isPlay)
    : m_level(level), m_left(isLeft), m_play(isPlay) {
...
void Field::start() {
    ...
    m_pHoleNumber = new HoleNumber();
    auto fieldPos = getPosition();

    auto fieldSize_u = getTexture()->getSize();
    glm::vec2 fieldSize(static_cast<float>(fieldSize_u.x),
                        static_cast<float>(fieldSize_u.y));

    m_pHoleNumber->setPosition(
        {fieldPos.x, fieldPos.y - fieldSize.y / 2 - 20, 2});

    m_pScene->addObject(m_pHoleNumber);
...
void Field::loadFromFile() {
    ...
    if (!f) {
        ...
        filename.erase(filename.end() -
                           static_cast<int>(std::to_string(m_level).size()) - 5,
                       filename.end());
        m_dontPlay = true;
        m_level = 1;
        filename += std::to_string(m_level) + ".json";
    ...
    glm::vec3 ballPos =
        glm::vec3(data["start_ball_pos"][0], data["start_ball_pos"][1], 0);
    ...
    if(!m_dontPlay || !m_play){
        m_pBall = new Ball();
        m_pBall->setPosition(ballPos);
        m_pScene->addObject(m_pBall);
        if (m_play) {
            m_pBall->player = true;
            m_pBall->addComponent(new PlayerScript());
        }
        ...
        if (m_pHoleNumber) {
            m_pHoleNumber->m_number = m_level;
        }
    }
}
...
void Field::fixedUpdate() {
    ...
    if (m_pHole != nullptr && m_pBall != nullptr) {
        if (glm::distance(m_pHole->getPosition(), m_pBall->getPosition()) <=
            7) {
            setLevel(m_level + 1);
        }
        if (neededReload) {
            ...
            glm::vec3 ballPos = glm::vec3(data["start_ball_pos"][0],
                                          data["start_ball_pos"][1], 0);
            ballPos = localToWorldPosition(ballPos);
            m_pBall->m_body->SetLinearVelocity({0, 0});
            m_pBall->setPosition(ballPos);
            m_pBall->m_body->SetTransform(
                {ballPos.x / SCALE, ballPos.y / SCALE}, 0);
        }
    }
}
